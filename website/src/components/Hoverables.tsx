...
export function HoverCard({ children, _className }: CardProps) {
    return (
        <PopoutBlock className={`${_className} flex-1 ${ Animations.riseOnHover } transition-transform`}>
            {children}
        </PopoutBlock>
    )
}
  ...
  return (
    <div className="group flex flex-col items-center justify-center gap-2 p-2 w-16 h-20 sm:w-20 sm:h-24 transition-transform">
      <img
        src={iconSrc}
        alt={name}
        className="w-10 h-10 sm:w-12 sm:h-12 object-contain saturate-50 opacity-50 group-hover:saturate-100 group-hover:scale-125 group-hover:opacity-100 group-hover:-translate-y-1 transition-all duration-300"
      />
